...
class AbstractPowerCalculator(object):
    ...
    DEBUG = False  # default value - can be overridden in config.py
    ...
    @staticmethod
    def set_debug(debug):
        AbstractPowerCalculator.DEBUG = debug
    ...
    def update(self, revs_per_sec):
        ...
        if self.DEBUG: print "cumulative_time(): " + repr(self.cumulative_time())
    ...
    def send_power(self):
        if self.DEBUG: print "send_power"
        ...
        if self.observer:
            ...
            if self.DEBUG: print "Power: ", repr(avePower)
        else:
